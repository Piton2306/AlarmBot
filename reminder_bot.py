...
async def process_day_callback(callback_query: types.CallbackQuery):
    ...
    await bot.edit_message_text(f"Текущий месяц: {months_ru[current_month]}\nВыберите месяц:", chat_id=chat_id, message_id=callback_query.message.message_id, reply_markup=builder.as_markup())
...
async def process_month_callback(callback_query: types.CallbackQuery):
    ...
    for hour in range(0, 24):
        for minute in range(0, 60, 15):
            builder.add(InlineKeyboardButton(text=f"{hour:02}:{minute:02}", callback_data=f"time_{hour:02}:{minute:02}"))
    builder.adjust(4)

    await bot.edit_message_text(f"Текущее время: {current_time}\nВыберите время:", chat_id=chat_id, message_id=callback_query.message.message_id, reply_markup=builder.as_markup())
...
async def process_time_callback(callback_query: types.CallbackQuery):
    ...
    await bot.edit_message_text("Введите сообщение для напоминания:", chat_id=chat_id, message_id=callback_query.message.message_id)

@dp.message()
async def handle_message(message: Message):
    chat_id = message.chat.id
    if chat_id in temp_data and 'day' in temp_data[chat_id] and 'month' in temp_data[chat_id] and 'time' in temp_data[chat_id]:
...
async def list_reminders(message: Message):
    ...
    if chat_id in reminders:
        ...
        for reminder_time, reminder_message in reminders_list:
            ...
            if remaining_time > 0:
                remaining_time_str = str(int(remaining_time // 3600)) + " часов " + str(int((remaining_time % 3600) // 60)) + " минут"
